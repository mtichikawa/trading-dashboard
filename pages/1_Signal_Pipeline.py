...
st.markdown(
    "How the arc converts raw market data into a single actionable signal — from price "
    "ingestion through indicator calculation to final fused output."
)
...
with col1:
    st.markdown(
        ...
        Live OHLCV bars for BTC, ETH, and SOL fetched from Kraken via ccxt and stored in PostgreSQL.
        Market events (volume spikes, large candles) are tagged. News headlines logged for the sentiment path.
        </div>
        """,
...
with col2:
    st.markdown(
        ...
        **Confidence** is highest when both paths agree, lowest when they diverge. A high-confidence
        BUY means technical indicators and sentiment are both positive.
    ...
    with legend_col:
        ...
        indicator_notes = [
            ("EMA Crossover", ind["ema_crossover"],
             "Positive when the fast EMA (12-period) is above the slow EMA (26-period). Trend direction."),
            ("RSI (14)", ind["rsi"],
             "Relative Strength Index, normalized to −1/+1. Negative suggests oversold; positive suggests overbought."),
            ("MACD", ind["macd"],
             "+1 on a bullish signal line crossover, −1 on bearish. Momentum shifts."),
